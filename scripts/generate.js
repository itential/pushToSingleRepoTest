...
const filePath = __dirname.replace("/scripts", pathvar);
const bundlePath = __dirname.replace("/scripts", "");
try {
  ...
  components.forEach((file) => {
    try {
      const data = fs.readFileSync(path.join(bundlePath, `${file.location}`), {
        encoding: "utf-8",
      });
